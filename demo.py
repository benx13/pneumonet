...
if 'resnet18masked' not in st.session_state:
    ...
    st.session_state.resnet18masked.load_state_dict(
        torch.load('pretrained_models/xresnet18masked.pt', map_location=torch.device('cpu')))


def trans(img, mean, std):
    tx = Compose([transforms.ToPILImage(),
...
def plot_hm(image, mean, std):
    ...
    z = image * torch.tensor(std).view(3, 1, 1)
    z = z + torch.tensor(mean).view(3, 1, 1)
    return getLabel(y_pred_tags.item()), overlay_mask(to_pil_image(z), to_pil_image(activation_map, mode='F'),
                                                      alpha=0.5)
...
# st.sidebar.info("This is a demo application written to help you understand Streamlit. The application identifies
# the animal in the picture. It was built using a Convolution Neural Network (CNN).") st.title('Select a model')
...
if st.button('No preprocessing'):
    ...
    y_tag, hm = plot_hm(trans(img, [0.1826, 0.1826, 0.1826], [0.2311, 0.2311, 0.2311]), [0.1826, 0.1826, 0.1826],
                        [0.2311, 0.2311, 0.2311])
